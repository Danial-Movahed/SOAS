...
class Management(ui_Management.Ui_MainWindow, QMainWindow):
    ...
    def __ViewHouse(self,item):
        if self.isRefreshing:
            return
        self.ShowAdWnd = ShowAdAdmin.ShowAdAdmin(DBConnection.execute(HouseTable.select().where(HouseTable.c.Title == self.HouseTable.item(self.HouseTable.currentRow(),0).text())).fetchall()[0][0])
